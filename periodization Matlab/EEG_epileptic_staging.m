function [out_schedule_path]=EEG_epileptic_staging(in_schedule_path, in_seizure_path, preictal_duration, interictal_ex_duration)
    ...
    for v=1:row_count
        ...
        for j = 1:height(currentGroup)
            ...
            if(ictal_data_index<=height(seizureGroup))
                ictal_row = seizureGroup(ictal_data_index, :);
            end
            % 针对每一行数据进行处理
            if strcmp(current_file, ictal_row.FileName)
                % 处理发作期间
                seizure_start_second = ictal_row.StartTime;
                seizure_end_second = ictal_row.EndTime;
                seizure_duration=seizure_end_second-seizure_start_second;
                file_duration = hourNumberToSecond(current_end_time - current_start_time);
        
                file_start_index = start_index;
                file_end_index = file_start_index + file_duration;
        
                end_index = start_index-1+seizure_start_second;
                % 标记发作前、发作期间和发作后的时间段
                initial_array(start_index:end_index) = 1;
                start_index = end_index+1;
                
                end_index = start_index+seizure_duration;
                initial_array(start_index:end_index) = -1;
                start_index = end_index+1;
                
                initial_array(start_index:file_end_index) = 1;
                start_index = file_end_index+1;
                
                ictal_data_index = ictal_data_index + 1;
            else
                % 处理非发作期间
                durationNonSeizure = hourNumberToSecond(current_end_time - current_start_time);
...
%         end
        ...
        temp_next_interictal_start_number = ictal_start_indices(:,k+1) - interictal_ex_duration_second;
            %情况二：间隔大于前期定义值，小于后期定义值
            if(ictal_start_indices(:,k+1)-ictal_start_indices(:,k)>30*60&&...
                ictal_start_indices(:,k+1)-ictal_start_indices(:,k)<=60*60)
                 % 保证前期有30分钟
                 preictal_start = ictal_start_indices(:, k + 1) - preictal_duration*60;
%                   发作结束到前期开始这段时间定义为后期
                  postictal_array(ictal_end_indices(:, k):preictal_start) = postictal_array(ictal_end_indices(:, k):preictal_start) * 3;
                 % 排除期结束到下一次发作开始定义为前期
                 preictal_array(preictal_start:ictal_start_indices(:, k + 1)) = preictal_array(preictal_start:ictal_start_indices(:, k + 1)) * 2;
%             %情况三：间隔大于后期定义值，小于2倍的后期定义值
%             elseif(ictal_start_indices(:,k+1)-ictal_start_indices(:,k)>interictal_ex_duration_second&&...
...
%         final_array = initial_array.*preictal_array.*postictal_array.*interictal_array.*excluded_array;
        final_array = initial_array.*preictal_array.*postictal_array.*interictal_array;
        start_index = 1;
...
%             
    ...
    %End
    % 构建文件名
    file_name = ['CHBMIT_seizures_new_', num2str(preictal_duration), '_', num2str(interictal_ex_duration), '.xlsx'];
